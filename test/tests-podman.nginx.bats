...
export BATS_PHP_DOCKER_IMAGE_NAME="${DOCKER_IMAGE_NAME:-docker.io/elasticms/base-php:8.0-nginx}"
...
@test "[$TEST_FILE] Starting LAMP stack services (nginx,mysql,php)" {
  command podman-compose -f ${BATS_TEST_DIRNAME%/}/docker-compose.nginx.yml up -d php mysql
}

@test "[$TEST_FILE] Check for startup messages in containers logs" {
  podman_wait_for_log php 60 "INFO success: nginx entered RUNNING state"
